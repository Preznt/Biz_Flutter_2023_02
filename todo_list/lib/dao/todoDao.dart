 ...
 * 모든 return type 를 Future<ReturnType> 로 감싸준다
 * 그러면 함수가 실행되면서 return  되는 데이터가
 * State 를 통해서 화면에 그릴 수 있도록 한다.
 */

class TodoDao {
  late Database _database;
  // 다른 곳에서 사용할때 getter 메서드를 사용하는 것
...
''';
  ...
  Future<Database> initDB() async {
    ...
    /**
     * 만약 dbPath 가 '/user/db' 라는 경로이다라고 하면
     * '/user/db/todo.db' 라는 이름의 실제 파일 경로를 만들어달라
     */
  ...
  Future<List<TodoDto>> selectAll() async {
    ...
    final List<Map<String, dynamic>> todoList =
        await _database.query("todo_table");
